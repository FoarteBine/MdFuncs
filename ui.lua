...
function MdFuncs:Init()
	...
	TabsHolder.Size = UDim2.new(1, -170, 1, 0); TabsHolder.Position = UDim2.new(0, 15, 0, 0)
	...
	local OverflowBtn = Instance.new("ImageButton", Main)
	OverflowBtn.Size = UDim2.new(0, 20, 0, 20)
	OverflowBtn.Position = UDim2.new(0, TabsHolder.AbsoluteSize.X + 25, 0.5, -10)
	OverflowBtn.BackgroundTransparency = 1
	OverflowBtn.Image = "rbxassetid://6034818372"
	OverflowBtn.ImageColor3 = Theme.TextDim
	OverflowBtn.Visible = false
	OverflowBtn.ZIndex = 10

	local OverflowMenu = Instance.new("Frame", Main)
	OverflowMenu.Size = UDim2.new(0, 140, 0, 0)
	OverflowMenu.Position = UDim2.new(0, TabsHolder.AbsoluteSize.X + 15, 1, 5)
	OverflowMenu.BackgroundColor3 = Theme.Main
	OverflowMenu.BorderSizePixel = 0
	OverflowMenu.ClipsDescendants = true
	OverflowMenu.Visible = false
	OverflowMenu.ZIndex = 20
	Instance.new("UICorner", OverflowMenu).CornerRadius = UDim.new(0, 6)
	local OMS = Instance.new("UIStroke", OverflowMenu); OMS.Color = Theme.Accent; OMS.Transparency = 0.6

	local OverflowLayout = Instance.new("UIListLayout", OverflowMenu)
	OverflowLayout.SortOrder = Enum.SortOrder.LayoutOrder
	OverflowLayout.Padding = UDim.new(0, 2)
	Instance.new("UIPadding", OverflowMenu).PaddingTop = UDim.new(0, 5)
	Instance.new("UIPadding", OverflowMenu).PaddingBottom = UDim.new(0, 5)
	Instance.new("UIPadding", OverflowMenu).PaddingLeft = UDim.new(0, 5)
	Instance.new("UIPadding", OverflowMenu).PaddingRight = UDim.new(0, 5)

	local isOverflowOpen = false
	OverflowBtn.MouseButton1Click:Connect(function()
		isOverflowOpen = not isOverflowOpen
		if isOverflowOpen then
			OverflowMenu.Visible = true
			TS:Create(OverflowBtn, TI, {Rotation = 180, ImageColor3 = Theme.Accent}):Play()
			TS:Create(OverflowMenu, TI, {Size = UDim2.new(0, 140, 0, OverflowLayout.AbsoluteContentSize.Y + 10)}):Play()
		else
			TS:Create(OverflowBtn, TI, {Rotation = 0, ImageColor3 = Theme.TextDim}):Play()
			TS:Create(OverflowMenu, TI, {Size = UDim2.new(0, 140, 0, 0)}):Play()
			task.delay(0.4, function() if not isOverflowOpen then OverflowMenu.Visible = false end end)
		end
	end)
	...
	local function UpdatePageSize()
		if CurrentPage and CurrentTabBtn then
			...
			local targetX = 0
			if CurrentTabBtn.Parent == TabsHolder then
				targetX = CurrentTabBtn.AbsolutePosition.X - Main.AbsolutePosition.X
			else
				targetX = 15
			end
			...
			TS:Create(CurrentPage, TI, {
				...
				Position = UDim2.new(0, targetX, 0, 56)
			}):Play()
		end
	end
	...
	RunService.RenderStepped:Connect(function()
		if CurrentPage and CurrentPage.Visible then
			 local targetX = 15
			 if CurrentTabBtn and CurrentTabBtn.Parent == TabsHolder then
				 targetX = CurrentTabBtn.AbsolutePosition.X - Main.AbsolutePosition.X
			 end
	...
	function Pages:Category(name)
		...
		task.defer(function()
			if not TabsHolder or not TabsLayout then return end
			TabsLayout:ApplyLayout()
			
			if TabsLayout.AbsoluteContentSize.X > TabsHolder.AbsoluteSize.X then
				TabBtn.Parent = OverflowMenu
				OverflowBtn.Visible = true
				...
				local HoverFrame = Instance.new("Frame", TabBtn)
				HoverFrame.ZIndex = -1
				...
				TabBtn.MouseEnter:Connect(function()
					if CurrentTabBtn ~= TabBtn then TS:Create(HoverFrame, TI, {BackgroundTransparency = 0.9}):Play() end
				end)
				TabBtn.MouseLeave:Connect(function()
					if CurrentTabBtn ~= TabBtn then TS:Create(HoverFrame, TI, {BackgroundTransparency = 1}):Play() end
				end)
			end
		end)
		...
		TabBtn.MouseButton1Click:Connect(function()
			...
			if CurrentPage == Page then
				...
				if Line.Visible then TS:Create(Line, TI, {Size = UDim2.new(0, 0, 0, 2)}):Play() end
				...
				if CurrentPage then
					...
					if CurrentTabBtn:FindFirstChild("Frame") and CurrentTabBtn:FindFirstChild("Frame").Visible then 
				...
				if Line.Visible then TS:Create(Line, TI, {Size = UDim2.new(1, 4, 0, 2)}):Play() end
			end
		end)

		if TabBtn.Parent == TabsHolder then
			TabBtn.MouseEnter:Connect(function()
